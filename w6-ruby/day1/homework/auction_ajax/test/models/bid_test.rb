...
class BidTest < ActiveSupport::TestCase
  ...
  test "bid can be created for above starting amount" do
    assert Bid.new(item: items(:hippo), amount: 500).save
  end

  test "bid cannot be created for below starting amount" do
    refute Bid.new(item: items(:hippo), amount: 1).save
  end

  test "bid cannot be created for below max bid" do
    assert Bid.new(item: items(:hippo), amount: 500).save
    refute Bid.new(item: items(:hippo), amount: 400).save
    refute Bid.new(item: items(:hippo), amount: 500).save
  end
